...
- **Long-running work**: launch async/background runs and inspect them later. Normally omit `timeoutMs`, `maxRuntimeMs`, `turnBudget`, and `toolBudget`; set one only for an explicit user request or concrete external constraint, never for speculative cost/runaway concerns. When authorized, `timeoutMs`/`maxRuntimeMs` set a run deadline, `turnBudget: { maxTurns, graceTurns }` sets a soft assistant-turn budget, and `toolBudget: { soft?, hard, block? }` can nudge and then block configured tools so the child can finalize
