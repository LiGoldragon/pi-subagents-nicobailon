...
describe("intercom result delivery cutover", { skip: !available ? "executor not importable" : undefined }, () => {
	...
	it("resume action revives completed multi-child async runs by index", async () => {
		...
		try {
			...
			assert.equal(args.includes("--model"), false, "revival must use the currently discovered agent authority, not persisted model settings");
		} finally {
			fs.rmSync(asyncDir, { recursive: true, force: true });
		}
	});
	...
	it("rejects revival when the current agent definition was removed", async () => {
		...
		try {
			...
			fs.writeFileSync(path.join(asyncDir, "recovery-descriptor.json"), JSON.stringify({
				version: 1, sourceRunId: runId, agent: "removed-worker", cwd: tempDir,
				model: "anthropic/claude-sonnet-4:high", tools: ["read"], systemPrompt: "Original persisted prompt",
				systemPromptMode: "replace", inheritProjectContext: false, inheritSkills: false,
				outputMode: "inline", maxSubagentDepth: 1, share: false,
			}, null, 2), "utf-8");
