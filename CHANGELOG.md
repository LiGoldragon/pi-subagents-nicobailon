...
### Changed
- Default the parent-facing `subagent` tool description and parameter schema to compact mode, preserving complete parameter descriptions in explicit `full` and valid `custom` modes. Budget controls now state that normal dispatch omits them and permits them only for explicit user requests or concrete external constraints, not speculative cost/runaway concerns.
