...
| `async` | boolean | true when omitted | Background execution. Omit for background; `async: false` is a non-Manager foreground exception. For non-Manager chains, `clarify: true` explicitly keeps the run foreground. |
...
Controls the parent-facing `subagent` tool description registered at startup. `compact` is the default minimal direct-launch surface: `{ agent, task?, async?, context? }`, plus recovery-only `{ action: "list" }`. Generated packets own role names and descriptions, so dispatch never requires a preliminary list call; list only when the roster is missing or stale, or a known-role launch fails, and its results are caller-filtered. Status/transcripts, wait, controls, chains/fanout, budgets, acceptance overrides, worktrees, scheduling, administration, watchdogs, packaged prompt workflows, and their parameter schemas are opt-in through `full` or `custom`. The measured minimal registered surface (description plus serialized schema) is 2,471 characters, reduced by 12,006 characters from the fork-main 14,477-character baseline.

`custom` reads `subagent-tool-description.md` from the project config directory, then from `~/.pi/agent/subagent-tool-description.md`. A valid custom mode retains the complete parameter schema; missing, empty, unreadable, or oversized custom files fall back to the full description and complete schema. Custom templates may use `{{fullDescription}}`, `{{compactDescription}}`, `{{safetyGuidance}}`, `{{agentDir}}`, and `{{projectConfigDir}}`; the safety guidance is always present so custom prose cannot remove the runtime guardrails. Restart Pi after changing the mode or custom file.
