...
): { scope?: ManagementScope; error?: AgentToolResult<Details> } {
...
function pickEffectiveAgent(
  d: ReturnType<typeof discoverAgentsAll>,
  name: string,
): AgentConfig | undefined {
  const raw = name.trim();
  const sanitized = sanitizeName(raw);
  const matches = allAgents(d).filter(
    (a) => a.name === raw || a.name === sanitized,
  );
  if (matches.length === 0) return undefined;
  return matches.reduce((best, agent) =>
    AGENT_SOURCE_PRECEDENCE[agent.source] > AGENT_SOURCE_PRECEDENCE[best.source]
      ? agent
      : best,
  );
}
...
function skillsWarning(
  cwd: string,
  agent: Pick<AgentConfig, "skills" | "skillPath" | "filePath">,
): string | undefined {
...
function editableAgentConfig(agent: AgentConfig): AgentConfig {
  ...
  return {
    ...
    subagentOnlyExtensions: base.subagentOnlyExtensions
      ? [...base.subagentOnlyExtensions]
      : undefined,
...
function preservedAgentFrontmatterFields(
  agent: AgentConfig,
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (hasKey(s, "outputSchema")) {
      if (typeof s.outputSchema === "string")
        ...
        return {
          error: `config.steps[${i}].outputSchema must be a schema file path string for saved chains.`,
        };
    }
    if (hasKey(s, "output")) {
    ...
    if (hasKey(s, "toolBudget")) {
      const validation = validateToolBudgetConfig(
        s.toolBudget,
        `config.steps[${i}].toolBudget`,
      );
...
function applyAgentConfig(
  ...
  if (hasKey(cfg, "skillPath")) {
    if (cfg.skillPath === false || cfg.skillPath === "")
      ...
      Array.isArray(cfg.skillPath) &&
      cfg.skillPath.every((entry) => typeof entry === "string")
    ) {
      const skillPath = [
        ...new Set(cfg.skillPath.map((entry) => entry.trim()).filter(Boolean)),
      ];
      target.skillPath = skillPath.length ? skillPath : undefined;
    } else
  ...
  if (hasKey(cfg, "acceptanceRole")) {
    if (cfg.acceptanceRole === false || cfg.acceptanceRole === "")
      ...
      cfg.acceptanceRole === "read-only" ||
      cfg.acceptanceRole === "writer"
    )
...
function resolveTarget<T extends { source: AgentSource; filePath: string }>(
  ...
  const mutable = matches.filter((m): m is T & { source: ManagementScope } =>
    isMutableSource(m.source),
  );
  if (mutable.length === 0) {
    if (matches.length > 0) {
      return result(
        `${kind === "agent" ? "Agent" : "Chain"} '${name}' is read-only and cannot be modified. Create a same-named ${kind} in user or project scope to override it.`,
        true,
      );
    }
...
function renamePath(
  ...
  const ext =
    kind === "agent"
      ? ".md"
      : currentPath.endsWith(".chain.json")
        ? ".chain.json"
        : ".chain.md";
...
function formatAgentDetail(agent: AgentConfig): string {
  ...
  if (agent.defaultTimeoutMs !== undefined)
    lines.push(
      `Legacy timeout frontmatter (not applied to launches): ${agent.defaultTimeoutMs}ms`,
    );
  if (agent.defaultTurnBudget)
    lines.push(
      `Legacy turn-budget frontmatter (not applied to launches): ${JSON.stringify(agent.defaultTurnBudget)}`,
    );
  if (agent.defaultAcceptance !== undefined)
    lines.push(
      `Acceptance: ${typeof agent.defaultAcceptance === "object" ? JSON.stringify(agent.defaultAcceptance) : String(agent.defaultAcceptance)}`,
    );
  ...
  if (agent.memory)
    lines.push(
      `Memory: ${agent.memory.scope} scope, path: ${agent.memory.path}`,
    );
...
function formatChainStepDetail(step: ChainStepConfig, index: number): string[] {
  ...
  if (step.expand || step.collect) {
    ...
    if (expand?.from)
      lines.push(
        `   Expand: ${String(expand.from.output ?? "?")}${String(expand.from.path ?? "")}`,
      );
    ...
    if (parallel && "toolBudget" in parallel)
      lines.push(
        `   Tool budget: ${JSON.stringify((parallel as { toolBudget?: unknown }).toolBudget)}`,
      );
...
function formatModelSource(
  ...
  if (
    agent.modelSource?.type === "subagents.defaultModel" &&
    agent.model === agent.modelSource.model
  ) {
...
function handleModels(
  ...
  if (
    requestedAgent &&
    !(BUILTIN_AGENT_NAMES as readonly string[]).includes(requestedAgent)
  ) {
    return result(
      `Builtin agent '${requestedAgent}' not found. Available: ${BUILTIN_AGENT_NAMES.join(", ")}.`,
      true,
    );
  }
  ...
  const builtinByName = new Map(
    discovered.builtin.map((agent) => [agent.name, agent]),
  );
  ...
  if (requestedAgent) {
    ...
    const resolvedModel = resolveSubagentModelOverride(
      agent.model,
      currentModel,
      availableModels,
      preferredProvider,
...
export function handleCreate(
  ...
  const targetDir = isChain
    ? scope === "user"
      ? d.userChainDir
      : (d.projectChainDir ?? path.join(projectConfigDir, "chains"))
    : scope === "user"
      ? d.userDir
      : (d.projectDir ?? path.join(projectConfigDir, "agents"));
...
export function handleUpdate(
  ...
  if (params.agent) {
    ...
    if (applyError) return result(applyError, true);
    const preserveFrontmatterFields = preservedAgentFrontmatterFields(
      target,
      cfg,
    );
    ...
    fs.writeFileSync(
      updated.filePath,
      serializeAgent(updated, { preserveFrontmatterFields }),
      "utf-8",
    );
    if (updated.name !== oldName) {
  ...
  fs.writeFileSync(
    updated.filePath,
    updated.filePath.endsWith(".chain.json")
      ? serializeJsonChain(updated)
      : serializeChain(updated),
    "utf-8",
  );
  const headline =
    updated.name === oldName
...
function handleEject(
  ...
  const d = discoverAgentsAll(ctx.cwd);
  const source = [...d.package, ...d.builtin].find(
    (a) => a.name === raw || a.name === sanitized,
  );
  if (!source) {
    return result(
      `Agent '${raw}' not found or is not a bundled/package agent. eject copies a builtin or package agent to ${scope} scope so it can be customized. Available: ${availableNames(ctx.cwd, "agent").join(", ") || "none"}.`,
      true,
    );
  }
  const runtimeName = source.name;
  const existingCustom = (scope === "user" ? d.user : d.project).find(
    (a) => a.name === runtimeName,
  );
  if (existingCustom) {
    return result(
      `Agent '${runtimeName}' is already a custom ${scope} agent at ${existingCustom.filePath}. Edit it with { action: "update", agent: "${runtimeName}" } or delete it first.`,
      true,
    );
  }
  if (nameExistsInScope(ctx.cwd, scope, runtimeName)) {
    return result(
      `An agent or chain named '${runtimeName}' already exists in ${scope} scope. Remove or rename it first.`,
      true,
    );
  }
  const projectConfigDir = getProjectConfigDir(ctx.cwd);
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    return result(
      `Failed to read source agent at ${source.filePath}: ${message}`,
      true,
    );
  }
  fs.writeFileSync(targetPath, content, "utf-8");
  return result(
    `Ejected agent '${runtimeName}' from ${source.source} to ${scope} scope at ${targetPath}. Edit it there to customize; it shadows the bundled ${source.source} agent of the same name.`,
  );
}
...
function handleDisable(
  ...
  if (scope === "project" && d.projectSettingsPath === null) {
    return result(
      "Project override is not available here: no project config root (.pi or .agents) was found above the cwd. Use agentScope: 'user' or run from inside a project.",
      true,
    );
  }
  ...
  if (!effective) {
    return result(
      `Agent '${raw}' not found. Available: ${availableNames(ctx.cwd, "agent").join(", ") || "none"}.`,
      true,
    );
  }
  const runtimeName = effective.name;
  const settingsPath = mergeBuiltinAgentOverride(ctx.cwd, runtimeName, scope, {
    disabled: true,
  });
  const after = pickEffectiveAgent(discoverAgentsAll(ctx.cwd), raw);
  if (after?.disabled === true) {
    return result(
      `Disabled agent '${runtimeName}' via ${scope} settings override at ${settingsPath}. It is now hidden from runtime discovery and { action: "list" }.`,
    );
  }
  return result(
    `Wrote a disabled override for '${runtimeName}' at ${settingsPath}, but the agent is still enabled. A higher-precedence ${after?.override?.scope ?? "project"} override is likely winning. Try agentScope: '${after?.override?.scope ?? "project"}'.`,
    true,
  );
}
...
function handleEnable(
  ...
  if (after && after.disabled !== true) {
    if (removed)
      return result(
        `Enabled agent '${runtimeName}' (removed disabled override at ${settingsPath}).`,
      );
    return result(`Agent '${runtimeName}' is already enabled.`);
  }
  if (after?.override?.scope && after.override.scope !== scope) {
    return result(
      `Agent '${runtimeName}' is still disabled via a ${after.override.scope} scope override at ${after.override.path}. Specify agentScope: '${after.override.scope}' to enable it.`,
      true,
    );
  }
  return result(
    `Agent '${runtimeName}' is still disabled after removing the ${scope} disabled override. It may be hidden via subagents.disableBuiltins in ${after?.override?.scope ?? scope} settings at ${after?.override?.path ?? settingsPath}.`,
    true,
  );
}
...
function handleReset(
  ...
  const bundled = [...d.package, ...d.builtin].find(
    (a) => a.name === raw || a.name === sanitized,
  );
  if (!bundled) {
    const custom = [...d.user, ...d.project].find(
      (a) => a.name === raw || a.name === sanitized,
    );
    if (custom) {
      return result(
        `Agent '${raw}' has no bundled default to reset to. Use { action: "delete", agent: "${custom.name}" } to remove the custom ${custom.source} agent.`,
        true,
      );
    }
    return result(
  ...
  const custom = (scope === "user" ? d.user : d.project).find(
    (a) => a.name === raw || a.name === sanitized,
  );
  const lines: string[] = [];
  if (custom) {
  ...
  const overrideRemoval = removeBuiltinAgentOverride(
    ctx.cwd,
    runtimeName,
    scope,
  );
  if (overrideRemoval.removed)
    lines.push(
      `Removed ${scope} settings override at ${overrideRemoval.path}.`,
    );
  if (lines.length === 0) {
    const otherScope = scope === "user" ? "project" : "user";
    const otherCustom = (otherScope === "user" ? d.user : d.project).find(
      (a) => a.name === raw || a.name === sanitized,
    );
    const hasOtherOverride = bundled.override?.scope === otherScope;
    const note =
      otherCustom || hasOtherOverride
        ? ` Customization exists in ${otherScope} scope; specify agentScope: '${otherScope}' to reset it.`
        : "";
    return result(
      `Agent '${runtimeName}' has no ${scope} customization to reset.${note} It is at its bundled ${bundled.source} default.`,
    );
  }
  lines.push(
    `Reset agent '${runtimeName}' to its bundled ${bundled.source} default.`,
  );
  return result(lines.join("\n"));
}
