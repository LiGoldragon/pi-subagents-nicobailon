...
export function interruptLiveAsyncResumeTarget(input: {
  ...
  try {
    ...
    return {
      ok: false,
      message: `Failed to interrupt async run ${asyncId}: ${message}`,
    };
  }
}
...
function validateResultFile(
  ...
  if (resultsValue !== undefined) {
    ...
    results = resultsValue.map((entry, index) => {
      ...
      const model = validateOptionalString(
        child,
        "model",
        resultPath,
        `results[${index}].model`,
      );
      const thinking = validateOptionalString(
        child,
        "thinking",
        resultPath,
        `results[${index}].thinking`,
      );
...
function validateStatusForResume(
  ...
  if (status.steps !== undefined) {
    ...
    status.steps.forEach((step, index) => {
      ...
      if (
        stepRecord.sessionFile !== undefined &&
        typeof stepRecord.sessionFile !== "string"
      )
        throw new Error(
          `Invalid async status '${source}': steps[${index}].sessionFile must be a string.`,
        );
      if (
        stepRecord.model !== undefined &&
        typeof stepRecord.model !== "string"
      )
        throw new Error(
          `Invalid async status '${source}': steps[${index}].model must be a string.`,
        );
      if (
        stepRecord.thinking !== undefined &&
        typeof stepRecord.thinking !== "string"
      )
        throw new Error(
...
export function readAsyncRecoveryDescriptor(
...
): SteeringRecoveryDescriptor | undefined {
  ...
  try {
    ...
    throw new Error(
      `Failed to parse async recovery descriptor '${descriptorPath}': ${getErrorMessage(error)}`,
      { cause: error instanceof Error ? error : undefined },
    );
  }
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new Error(
      `Invalid async recovery descriptor '${descriptorPath}': expected an object.`,
    );
  ...
  for (const field of Object.keys(parsed)) {
    if (!allowedFields.has(field))
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': unknown field '${field}'.`,
      );
  }
  ...
  for (const field of requiredStrings) {
    if (typeof parsed[field] !== "string" || !(parsed[field] as string).trim())
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': ${field} must be a non-empty string.`,
      );
  }
  if (parsed.version !== 1)
    throw new Error(
      `Invalid async recovery descriptor '${descriptorPath}': version must be 1.`,
    );
  if (
    ...
    throw new Error(
      `Invalid async recovery descriptor '${descriptorPath}': systemPromptMode is invalid.`,
    );
  if (parsed.outputMode !== "inline" && parsed.outputMode !== "file-only")
    throw new Error(
      `Invalid async recovery descriptor '${descriptorPath}': outputMode is invalid.`,
    );
  for (const field of [
    "inheritProjectContext",
    "inheritSkills",
    ...
    if (typeof parsed[field] !== "boolean")
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': ${field} must be a boolean.`,
      );
  }
  if (
    !Number.isInteger(parsed.maxSubagentDepth) ||
    (parsed.maxSubagentDepth as number) < 0
  )
    throw new Error(
  ...
  if (
    ...
    throw new Error(
      `Invalid async recovery descriptor '${descriptorPath}': systemPrompt must be a string.`,
    );
  for (const field of [
    "sessionFile",
    "model",
    ...
    if (
      ...
      (typeof parsed[field] !== "string" || !(parsed[field] as string).trim())
    )
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': ${field} must be a non-empty string.`,
      );
  ...
  if (
    parsed.completionGuard !== undefined &&
    typeof parsed.completionGuard !== "boolean"
  )
    throw new Error(
      `Invalid async recovery descriptor '${descriptorPath}': completionGuard must be a boolean.`,
    );
  if (parsed.memory !== undefined) {
    if (
      ...
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': memory must be an object.`,
      );
    ...
    if (
      ...
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': memory is invalid.`,
      );
  }
  if (
    parsed.absoluteDeadlineAt !== undefined &&
    (!Number.isFinite(parsed.absoluteDeadlineAt) ||
      (parsed.absoluteDeadlineAt as number) <= 0)
  )
    throw new Error(
  ...
  if (parsed.initialTurnBudget !== undefined) {
    const result = resolveTurnBudgetConfig(
      parsed.initialTurnBudget,
      "recoveryDescriptor.initialTurnBudget",
    );
    if (result.error)
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': ${result.error}`,
      );
  }
  if (parsed.initialToolBudget !== undefined) {
    const result = validateToolBudgetConfig(
      parsed.initialToolBudget,
      "recoveryDescriptor.initialToolBudget",
    );
    if (result.error)
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': ${result.error}`,
  ...
  if (parsed.maxOutput !== undefined) {
    if (
      !parsed.maxOutput ||
      typeof parsed.maxOutput !== "object" ||
      Array.isArray(parsed.maxOutput)
    )
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': maxOutput must be an object.`,
      );
  ...
  if (parsed.artifactConfig !== undefined) {
    if (
      ...
      typeof parsed.artifactConfig !== "object" ||
      Array.isArray(parsed.artifactConfig)
    )
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': artifactConfig must be an object.`,
      );
    ...
    if (
      artifact.includeTranscript !== undefined &&
      typeof artifact.includeTranscript !== "boolean"
    )
      throw new Error(
    ...
    if (
      ...
      (artifact.cleanupDays as number) < 0
    )
      throw new Error(
  ...
  if (parsed.controlConfig !== undefined) {
    if (
      ...
      typeof parsed.controlConfig !== "object" ||
      Array.isArray(parsed.controlConfig)
    )
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': controlConfig must be an object.`,
      );
    ...
    if (
      !Number.isInteger(control.failedToolAttemptsBeforeAttention) ||
      (control.failedToolAttemptsBeforeAttention as number) < 1
    )
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': controlConfig.failedToolAttemptsBeforeAttention must be a positive integer.`,
      );
    if (
      !Array.isArray(control.notifyOn) ||
      control.notifyOn.some((item) => item !== "needs_attention")
    )
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': controlConfig.notifyOn is invalid.`,
      );
    if (
      !Array.isArray(control.notifyChannels) ||
      control.notifyChannels.some(
        (item) => item !== "event" && item !== "async" && item !== "intercom",
      )
    )
      throw new Error(
  ...
  if (parsed.acceptance !== undefined) {
    ...
    if (errors.length)
      throw new Error(
        `Invalid async recovery descriptor '${descriptorPath}': ${errors.join(" ")}`,
      );
  }
...
export function resolveAsyncResumeTarget(
  ...
  if (recoveryDescriptor && recoveryDescriptor.sourceRunId !== runId)
    throw new Error(
      `Async run '${runId}' has a recovery descriptor for a different source run.`,
    );
  ...
  if (state === "stopped")
    throw new Error(
      `Async run '${runId}' was stopped and cannot be resumed. Start a new run instead.`,
    );
  ...
  const resolvedSessionFile = sessionFile
    ? validateResumeSessionFile(runId, sessionFile)
    : undefined;
  const stepModel =
    statusSteps[index]?.model ??
    resultSteps[index]?.model ??
    (stepCount === 1 ? result?.model : undefined);
  const stepThinking =
    statusSteps[index]?.thinking ??
    resultSteps[index]?.thinking ??
    (stepCount === 1 ? result?.thinking : undefined);
...
export function applySteeringRecoveryAgentConfig(
  ...
  _descriptor: SteeringRecoveryDescriptor,
): AgentConfig {
