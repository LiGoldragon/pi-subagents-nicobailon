...
function rememberFleetJob(state: SubagentState, job: AsyncJobState): void {
  ...
  for (const stale of terminal.slice(MAX_RECENT_FLEET_JOBS))
    state.fleetJobs.delete(stale.asyncId);
}
...
export function createAsyncJobTracker(
  ...
  const summaryToJob = (run: AsyncRunSummary): AsyncJobState => {
    ...
    return {
      ...
      runningSteps: visibleSteps.filter((step) => step.status === "running")
        .length,
  ...
  const emitNewControlEvents = (job: AsyncJobState) => {
    ...
    try {
      ...
      const startedFromTail =
        ...
        stat.size > CONTROL_EVENT_SCAN_WINDOW_BYTES;
      if (startedFromTail) cursor = stat.size - CONTROL_EVENT_SCAN_WINDOW_BYTES;
      if (stat.size <= cursor) return;
      const scanEnd = Math.min(
        stat.size,
        cursor + CONTROL_EVENT_SCAN_WINDOW_BYTES,
      );
      const handleLine = (line: string) => {
        ...
        if (
          (parsed as { type?: unknown }).type === "subagent.steering.notice"
        ) {
          ...
          if (
            ...
            (notice.state !== "failed" &&
              ...
              notice.state !== "recovered") ||
            typeof notice.message !== "string"
          )
            return;
          if (
            typeof state.currentSessionId === "string" &&
            notice.currentSessionId !== state.currentSessionId
          )
            return;
          ...
          if (steeringNoticeSeen.size > 200) {
            for (const [seenKey, seenAt] of steeringNoticeSeen) {
              if (
                now - seenAt > 10 * 60 * 1000 ||
                steeringNoticeSeen.size > 200
              )
      ...
      while (readCursor < scanEnd) {
        const toRead = Math.min(
          CONTROL_EVENT_READ_CHUNK_BYTES,
          scanEnd - readCursor,
        );
      ...
      for (const job of state.asyncJobs.values()) {
        ...
        try {
          ...
          if (status) {
            ...
            job.turnBudgetExceeded =
              status.turnBudgetExceeded ?? job.turnBudgetExceeded;
  ...
  const handleStarted = (data: unknown) => {
    ...
    if (
      typeof state.currentSessionId === "string" &&
      info.sessionId !== state.currentSessionId
    )
      return;
    const now = Date.now();
  ...
  const restoreActiveJobs = (ctx?: ExtensionContext) => {
    ...
    try {
      ...
      console.error(
        `Failed to restore active async jobs from '${asyncDirRoot}':`,
        error,
      );
      return;
    }
